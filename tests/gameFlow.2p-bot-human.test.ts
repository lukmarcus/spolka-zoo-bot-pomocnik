...
/**
 * Complete Game Flow: Bot(red) + Human(yellow) - Shared Mode
 *
 * End-to-end test covering full game from START to END (5 rounds)
 * Bot first, skips human, triggers NEXT_PHASE on each NEXT_PLAYER
 */
...
describe("Complete Game Flow: Bot(red) + Human(yellow) - Shared Mode", () => {
  let state: GameState;
  ...
  it("1. Should initialize game with START_ADVANCED_GAME and show beforeFirstRound modal", () => {
    ...
    // 1 bot (bot + human), should use "single" message
  ...
  it("2. Should progress through phases and show nextPhase modal", () => {
    state = applyAction(state, {
      type: "START_ADVANCED_GAME",
      payload: { players, mode: "shared", modules },
    });
