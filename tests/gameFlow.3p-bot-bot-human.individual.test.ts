...
/**
 * Complete Game Flow: 3 Players (Bot + Bot + Human) - Individual Mode
 *
 * End-to-end test covering full game from START to END (5 rounds)
 * 2 bots with individual deck, human player auto-skipped
 * maxPhases = 3 for 3+ players
 * Rotation: Bot1 → Bot2 → Human (skip) → Phase2 (reset) → repeat
 */
...
describe("Complete Game Flow: 3 Players (Bot + Bot + Human) - Individual Mode", () => {
  let state: GameState;
  ...
  it("2. Should progress with nextBot and nextPhase modals (Bot1 → Bot2 → Phase2)", () => {
    ...
    // Bot2 → Phase2 (no more bots in phase 1, so NEXT_PHASE is called)
    state = applyAction(state, { type: "NEXT_PLAYER" });
    expect(state.currentPhase).toBe(2);
    expect(state.currentBot).toBe(1); // Reset to first bot
  ...
  it("4. Should shuffle deck on round transition", () => {
    ...
    // Progress to round 2 (3 phases × 3 players per phase)
    for (let phase = 1; phase <= 3; phase++) {
      state = applyAction(state, { type: "NEXT_PLAYER" });
      state = applyAction(state, { type: "NEXT_PLAYER" });
      state = applyAction(state, { type: "NEXT_PLAYER" });
  ...
  it("5. Should complete all 5 rounds and verify final game state", () => {
    ...
    // Rounds 1-4: Full progression (3 NEXT_PLAYER per phase × 3 phases)
