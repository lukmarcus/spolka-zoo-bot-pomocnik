...
/**
 * Complete Game Flow: Bot(red) + Bot(yellow) - Individual Mode
 *
 * End-to-end test covering full game from START to END (5 rounds)
 * 2 bots with individual decks
 */
...
describe("Complete Game Flow: Bot(red) + Bot(yellow) - Individual Mode", () => {
  let state: GameState;
  ...
  it("1. Should initialize game with START_ADVANCED_GAME and show beforeFirstRound modal", () => {
    ...
    // With 2 bots (bot + bot), should use "multiple" message
  ...
  it("2. Should progress through phases with 2 bots and show nextPhase modal", () => {
    ...
    // After START: Bot 1 at index 0, Bot 2 at index -1 (hasn't drawn)
    expect(state.botDecks![0].currentCardIndex).toBe(0); // Bot 1 auto-drew
    expect(state.botDecks![1].currentCardIndex).toBe(-1); // Bot 2 hasn't drawn

    // Move to Bot 2 (individual mode: DRAW_CARD happens when switching!)
    state = applyAction(state, { type: "NEXT_PLAYER" });
    expect(state.currentBot).toBe(2);
    expect(state.currentPhase).toBe(1);
    expect(state.currentPlayerIndex).toBe(1); // Now at Bot 2

    // In individual mode, when switching to Bot 2, it auto-draws!
    // So Bot 2 goes from -1 to 0
    expect(state.botDecks![0].currentCardIndex).toBe(0); // Bot 1 unchanged
    expect(state.botDecks![1].currentCardIndex).toBe(0); // Bot 2 drew on switch!
    ...
    // NEXT_PHASE auto-draws for Bot 1, but Bot 2 doesn't get drawn in this phase
    expect(state.botDecks![0].currentCardIndex).toBe(1); // Bot 1 drew
    expect(state.botDecks![1].currentCardIndex).toBe(0); // Bot 2 still at 0 from switch
  ...
  it("3. Should transition to round 2 and show nextRound modal", () => {
    ...
    // In individual mode after NEXT_ROUND:
    // Bot 1 gets auto-DRAW_CARD → index 0
    // Bot 2 hasn't drawn yet → index -1 (same as initial state)
    expect(state.botDecks![0].currentCardIndex).toBe(0); // Bot 1 auto-drew
    expect(state.botDecks![1].currentCardIndex).toBe(-1); // Bot 2 hasn't drawn

    // Modal: nextRound should be visible
    ...
    // With 2 bots (individual mode), should use note3.multiple_individual
  ...
  it("4. Should reset and shuffle individual decks on round transition", () => {
    state = applyAction(state, {
      type: "START_ADVANCED_GAME",
      payload: { players, mode: "individual", modules },
    });
    ...
    // Individual decks should be shuffled
    expect(state.botDecks!.length).toBe(2);
    expect(state.botDecks![0].cardSequence.length).toBe(13);
    expect(state.botDecks![1].cardSequence.length).toBe(13);

    // In individual mode after NEXT_ROUND:
    // Bot 1 gets auto-DRAW_CARD → index 0
    // Bot 2 hasn't drawn yet → index -1
    expect(state.botDecks![0].currentCardIndex).toBe(0); // Auto-drew in new round
    expect(state.botDecks![1].currentCardIndex).toBe(-1); // Not drawn yet
  });
  ...
  it("5. Should complete all 5 rounds and verify final game state", () => {
    ...
    for (let round = 1; round <= 4; round++) {
      for (let phase = 1; phase <= 4; phase++) {
        ...
        state = applyAction(state, { type: "NEXT_PLAYER" }); // Bot 1
        if (phase < 4) {
          state = applyAction(state, { type: "NEXT_PLAYER" }); // Bot 2 -> NEXT_PHASE
        } else {
          state = applyAction(state, { type: "NEXT_PLAYER" }); // Bot 2 -> NEXT_PHASE -> NEXT_ROUND
        }
      }
    }
